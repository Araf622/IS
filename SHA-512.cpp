...
string dec_to_hex(lli deci)
{
	...
	for (unsigned int i = 0;
		...
		hexstring += bin_to_hex(temp);
	}

	return hexstring;
}

lli BintoDec(string bin)
{

	lli value = bitset<64>(bin)
					.to_ullong();
	return value;
}

// Function to right rotate x by n bits
lli rotate_right(lli x, int n)
{
	return (x >> n) | (x << (64 - n));
}


lli shift_right(lli x, int n)
{
	return (x >> n);
}
...
void separator(string getBlock)
{
	...
	for (int g = 16; g < 80; ++g) {
		...
		lli WordA = rotate_right(Message[g - 2], 19) ^ rotate_right(Message[g - 2], 61) ^ shift_right(Message[g - 2], 6);

		lli WordB = Message[g - 7];

		// Find the WordC
		lli WordC = rotate_right(Message[g - 15], 1)
					^ rotate_right(Message[g - 15], 8)
					^ shift_right(Message[g - 15], 7);

		// Find the WordD
		lli WordD = Message[g - 16];

		// Find the resultant code
		lli T = WordA + WordB + WordC + WordD;
...
void Func(lli a, lli b, lli c,
		lli& d, lli e, lli f,
		lli g, lli& h, int K)
{
	// Find the Hash Code
	lli T1 = h + Ch(e, f, g) + sigmaE(e) + Message[K]
			+ Constants[K];
	lli T2 = sigmaA(a) + maj(a, b, c);

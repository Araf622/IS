...
string expanded_key[176];
int z = 4;

string round_const[10] = {"01","02","04","08","10","20","40","80","1B","36"};
...
string bin_to_hex(string bin){
        ...
        while(bin[i]){
            string str = bin.substr(i, 4);
            if      (!str.compare("0000")) hex += "0";
            else if (!str.compare("0001")) hex += "1";
            else if (!str.compare("0010")) hex += "2";
            else if (!str.compare("0011")) hex += "3";
            else if (!str.compare("0100")) hex += "4";
            else if (!str.compare("0101")) hex += "5";
            else if (!str.compare("0110")) hex += "6";
            else if (!str.compare("0111")) hex += "7";
            else if (!str.compare("1000")) hex += "8";
            else if (!str.compare("1001")) hex += "9";
            else if (!str.compare("1010")) hex += "A";
            else if (!str.compare("1011")) hex += "B";
            else if (!str.compare("1100")) hex += "C";
            else if (!str.compare("1101")) hex += "D";
            else if (!str.compare("1110")) hex += "E";
            else if (!str.compare("1111")) hex += "F";
            else continue;
            i += 4;
    }
    return hex;
...
struct Word addRoundKey(int r){
    ...
    //adding round const
    t.bytes[0] = hex_to_bin(t.bytes[0]);
    string temp2 = round_const[r-1];
    temp2 = hex_to_bin(temp2);
    t.bytes[0] = x_or(t.bytes[0],temp2);
    t.bytes[0] = bin_to_hex(t.bytes[0]);
...
struct Word x_or_word(Word word1,Word word2){
    Word w;
    word1.bytes[0] = hex_to_bin(word1.bytes[0]);
    word1.bytes[1] = hex_to_bin(word1.bytes[1]);
    word1.bytes[2] = hex_to_bin(word1.bytes[2]);
    word1.bytes[3] = hex_to_bin(word1.bytes[3]);

    word2.bytes[0] = hex_to_bin(word2.bytes[0]);
    word2.bytes[1] = hex_to_bin(word2.bytes[1]);
    word2.bytes[2] = hex_to_bin(word2.bytes[2]);
    word2.bytes[3] = hex_to_bin(word2.bytes[3]);
    ...
    w.bytes[0] = bin_to_hex(w.bytes[0]);
    w.bytes[1] = bin_to_hex(w.bytes[1]);
    w.bytes[2] = bin_to_hex(w.bytes[2]);
    w.bytes[3] = bin_to_hex(w.bytes[3]);
...
void mixColumn(){
 ...
 //   string mixMatrix[4][4];
    ...
    for(int i = 0;i < 16;i++){
            if(i%4 == 0){
            temp1 = x_or(hex_to_bin(table_two[hex_to_dec(state[i])]),hex_to_bin(table_three[hex_to_dec(state[i+1])]));
            temp2 = x_or(hex_to_bin(state[i+2]),hex_to_bin(state[i+3]));
            state2[i] = x_or(temp1,temp2);
            state2[i] = bin_to_hex(state2[i]);
            }
            if(i%4 == 1){
            temp1 = x_or(hex_to_bin(table_two[hex_to_dec(state[i])]),hex_to_bin(table_three[hex_to_dec(state[i+1])]));
            temp2 = x_or(hex_to_bin(state[i+2]),hex_to_bin(state[i-1]));
            state2[i] = x_or(temp1,temp2);
            state2[i] = bin_to_hex(state2[i]);
            }
            if(i%4 == 2){
            temp1 = x_or(hex_to_bin(table_two[hex_to_dec(state[i])]),hex_to_bin(table_three[hex_to_dec(state[i+1])]));
            temp2 = x_or(hex_to_bin(state[i-1]),hex_to_bin(state[i-2]));
            state2[i] = x_or(temp1,temp2);
            state2[i] = bin_to_hex(state2[i]);
            }
            if(i%4 == 3){
            temp1 = x_or(hex_to_bin(table_two[hex_to_dec(state[i])]),hex_to_bin(table_three[hex_to_dec(state[i-3])]));
            temp2 = x_or(hex_to_bin(state[i-1]),hex_to_bin(state[i-2]));
            state2[i] = x_or(temp1,temp2);
            state2[i] = bin_to_hex(state2[i]);
            }
...
void addRoundKey_matrix(){
     ...
     for(int i = 0;i<4;i++){
        for(int j = 0;j<4;j++){
            string temp = x_or(hex_to_bin(stateMatrix[j][i]),hex_to_bin(word[z].bytes[k++]));
            stateMatrix[j][i] = bin_to_hex(temp);
            if(k == 4){
                k = 0;
                z++;
            }
...
void AES(){
    ...
    for(int i = 0;i<4;i++){
        for(int j = 0;j<4;j++){
            string temp1 = hex_to_bin(stateMatrix[j][i]);
            string temp2 = hex_to_bin(word[i].bytes[j]);
            stateMatrix[j][i] = x_or(temp1,temp2);
            stateMatrix[j][i] = bin_to_hex(stateMatrix[j][i]);
        }
    }
...
int main(){
    ...
    int l = 0,r = 0,c = 0;
    while(plaintext[l]){
    string str = dec_to_hex(plaintext[l]);
    reverse(str.begin(),str.end());
    stateMatrix[r++][c] = str;
    if(r == 4){
        c++;
